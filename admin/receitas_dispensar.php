...
<head>
    ...
    <!-- Open Graph -->
    <meta property="og:title" content="<?php echo htmlspecialchars($pageTitle); ?> - Farmácia Popular">
                        ...
                        <?php foreach ($itens as $item): ?>
                            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-5 border-2 border-blue-200">
                                <div class="flex items-start justify-between mb-3">
                                    ...
                                    <span class="inline-block px-3 py-1 bg-white rounded-full text-sm font-bold text-gray-700 border-2 border-blue-300">
                                        Estoque: <?php echo $item['estoque_atual']; ?>
                                    </span>
                                </div>
                                ...
                                <?php if ($item['quantidade_disponivel'] > 0 && $receita['status'] === 'ativa'): ?>
                                    <div class="mt-4">
                                        <button 
                                            onclick="abrirModalDispensacao(<?php echo $item['id']; ?>, '<?php echo addslashes($item['medicamento_nome']); ?>', <?php echo $item['quantidade_disponivel']; ?>, <?php echo $item['medicamento_id']; ?>)"
                                            class="w-full px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded-lg font-bold shadow-lg hover:shadow-xl transition-all">
    ...
    <div id="modalDispensacao" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl p-6 max-w-2xl w-full mx-4 shadow-2xl">
            ...
            <form id="formDispensacao" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        <span class="text-red-500">*</span> Quantidade a Dispensar
                    </label>

        ...
        function renderizarPacientes(pacientes) {
            ...
            container.innerHTML = `
                                    ...
                                    <tr class="text-sm text-slate-600 hover:bg-blue-50/50 transition-colors cursor-pointer" onclick="window.location.href='paciente_historico.php?id=${paciente.id}'" title="Clique para ver o histórico do paciente">
                                        <td class="px-6 py-4">
                                            <div class="flex flex-col">
                                                ...
                                                ${paciente.total_saidas > 0 ? `<span class="text-xs text-slate-400 mt-1">${paciente.total_saidas} saídas registradas</span>` : ''}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4" onclick="event.stopPropagation();">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <form method="post" class="inline" onsubmit="event.stopPropagation(); return confirm('Confirma a exclusão do paciente ${paciente.nome.replace(/'/g, "\\'")}? Esta ação não poderá ser desfeita.');">
                                                    <input type="hidden" name="delete_paciente" value="1">
                ...
                <div class="lg:hidden divide-y divide-slate-100">
                    ${pacientes.map(paciente => {
                        return `
                            <div class="p-5 bg-white/80 space-y-3 hover:bg-blue-50/50 transition-colors cursor-pointer" onclick="window.location.href='paciente_historico.php?id=${paciente.id}'" title="Clique para ver o histórico do paciente">
                                <div class="flex items-start justify-between">
                                    <div>
                                        <p class="text-xs text-slate-500 uppercase tracking-wide">Nome</p>
                                        <p class="font-semibold text-slate-900">${paciente.nome}</p>
                                        ${paciente.total_saidas > 0 ? `<p class="text-xs text-slate-400 mt-1">${paciente.total_saidas} saídas</p>` : ''}
                                    </div>
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${sexoBadgeClass(paciente.sexo)}">
                                        ${sexoLabel(paciente.sexo)}
                                    </span>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <p class="text-xs text-slate-500">Data de Nascimento</p>
                                        <p class="text-sm text-slate-700">${formatarData(paciente.data_nascimento)}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-slate-500">Idade</p>
                                        <p class="text-sm text-slate-700">${paciente.idade || 0} anos</p>
                                    </div>
                                </div>
                                ...
                                ${paciente.cpf ? `
                                    <div>
                                        <p class="text-xs text-slate-500">CPF</p>
                                        <p class="text-sm font-mono text-slate-700">${paciente.cpf}</p>
                                    </div>
                                ` : ''}
                                ...
                                ${paciente.cartao_sus ? `
                                    <div>
                                        <p class="text-xs text-slate-500">Cartão SUS</p>
                                        <p class="text-sm font-mono text-slate-700">${paciente.cartao_sus}</p>
                                    </div>
                                ` : ''}

                                <div class="flex gap-2 pt-2 border-t border-slate-100" onclick="event.stopPropagation();">
                                    <a href="pacientes_form.php?id=${paciente.id}" class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium transition-all shadow-sm hover:shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.862 4.487l1.687 1.688a1.875 1.875 0 102.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg>
                                        Editar
                                    </a>
                                    <form method="post" class="flex-1" onsubmit="event.stopPropagation(); return confirm('Confirma a exclusão do paciente?');">

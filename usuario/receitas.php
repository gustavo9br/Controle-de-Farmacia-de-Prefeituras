...
if (!empty($status) && in_array($status, ['ativa', 'finalizada', 'vencida', 'cancelada'], true)) {
...
try {
    $countSql = "SELECT COUNT(*) FROM receitas r INNER JOIN pacientes p ON p.id = r.paciente_id $whereClause";
    ...
    $dataSql = "SELECT r.*, p.nome as paciente_nome, p.cpf as paciente_cpf,
...
{
    return match ($status) {
        'ativa' => 'bg-emerald-100 text-emerald-600',
        'finalizada' => 'bg-blue-100 text-blue-600',
                ...
                <?php if (!empty($receitas)): ?>
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold <?php echo statusBadgeClass($receita['status']); ?>"><?php echo statusLabel($receita['status']); ?></span>
                                    </div>
                    ...
                    <?php if ($totalPages > 1): ?>
                        <nav class="flex items-center justify-center gap-2 mt-8">
                            <?php $queryParams = $_GET; for ($i = 1; $i <= $totalPages; $i++): ?>
                                <a href="receitas.php?<?php echo http_build_query(array_merge($queryParams, ['page' => $i])); ?>" class="pagination-chip <?php echo $i === $page ? 'active' : ''; ?>"><?php echo $i; ?></a>
                            <?php endfor; ?>
                        </nav>
                    <?php endif; ?>
                <?php else: ?>

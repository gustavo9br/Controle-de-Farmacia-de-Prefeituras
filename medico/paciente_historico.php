        ...
        <main class="flex-1 px-4 py-6 sm:px-6 sm:py-8 lg:px-12 lg:py-10 space-y-8">
                    ...
                    <div class="space-y-3">
                        <?php foreach ($receitas_ativas as $receita): ?>
                            <div class="rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm text-slate-600">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <div>
                                        <strong class="text-slate-900">Receita #<?php echo $receita['id']; ?></strong>
                                        <span class="ml-2 text-xs text-slate-400">Emitida em <?php echo date('d/m/Y', strtotime($receita['data_emissao'])); ?></span>
                                    </div>
                                    <span class="inline-flex items-center gap-2 rounded-full <?php echo $receita['itens_pendentes'] > 0 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'; ?> px-3 py-1 text-xs font-semibold">
                                        <?php echo $receita['itens_pendentes'] > 0 ? 'Itens pendentes' : 'Todos retirados'; ?>
                                    </span>
                                </div>
                                <div class="text-xs text-slate-400 mt-1">
                                    Validade: <?php echo date('d/m/Y', strtotime($receita['data_validade'])); ?> • Itens autorizados: <?php echo $receita['total_itens']; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <section class="glass-card p-6 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Últimas 50 ocorrências</span>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-100 text-left">
                        <thead class="bg-white/60">
                            <tr class="text-xs font-semibold uppercase tracking-wide text-slate-500">
                                ...
                                <th class="px-4 sm:px-6 py-3">Responsável</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white/80">
                            <?php foreach ($historico as $registro): ?>
                                <tr class="text-sm text-slate-600">
                                    <td class="px-4 sm:px-6 py-3 sm:py-4 text-slate-500"><?php echo date('d/m/Y H:i', strtotime($registro['data_dispensacao'])); ?></td>
                                    <td class="px-4 sm:px-6 py-3 sm:py-4">
                                        <div class="font-semibold text-slate-900"><?php echo htmlspecialchars($registro['medicamento_nome']); ?></div>
                                        <div class="text-xs text-slate-400"><?php echo htmlspecialchars($registro['apresentacao']); ?></div>
                                    </td>
                                    <td class="px-4 sm:px-6 py-3 sm:py-4 font-semibold text-slate-900"><?php echo (int) $registro['quantidade']; ?></td>
                                    <td class="px-4 sm:px-6 py-3 sm:py-4 text-slate-500"><?php echo htmlspecialchars($registro['lote_numero']); ?></td>
                                    <td class="px-4 sm:px-6 py-3 sm:py-4 text-slate-500"><?php echo htmlspecialchars($registro['usuario_nome']); ?></td>
                                    <td class="px-4 sm:px-6 py-3 sm:py-4">
                                        <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600"><?php echo htmlspecialchars($registro['tipo_dispensacao']); ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            <?php if (empty($historico)): ?>
                                <tr>
                                    <td colspan="6" class="px-6 py-5 text-center text-sm text-slate-400">Nenhuma dispensação registrada para este paciente.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
